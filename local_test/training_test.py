...
for index, config in enumerate(param_list):
    vec_env = make_vec_env(lambda: SnakeGameEnv(**config), n_envs=32)
    ...
    for i in range(40):
        model.learn(100000, progress_bar=True, tb_log_name=f"ppo_snake4.8_{index}", reset_num_timesteps=False)
        model.save(f'ppo_snake4.8_{index}.zip')