...
model = PPO('CnnPolicy', vec_env,verbose=False, device='cuda', tensorboard_log=log_dir, n_steps=128, batch_size=2048, learning_rate=0.0003, gae_lambda=0.99)

for i in range(40):
    model.learn(100000, progress_bar=True, tb_log_name=f"ppo_snake_6gs", reset_num_timesteps=False)
    model.save(f'ppo_snake_6gs.zip')